...
- **Crypto Universe**: 9 coins (BTC, ETH, SOL, BNB, ADA, AVAX, LINK, DOT, XRP). Quality/volume-gated by design — top-cap L1s and major smart-contract platforms only. Deliberate exclusion of memecoins (DOGE, SHIB, PEPE, etc.) and low-float altcoins. Any future crypto strategy (e.g. the Account 5 reversal candidate) must reuse this same 9-coin list or a subset; no expansion into thin-liquidity or narrative-speculation coins. Rationale: the crypto thesis is cross-sectional factor alpha on liquid names, not narrative-basket speculation — the latter would require microstructure/sentiment infrastructure we don't have, and exposes the book to single-name rugpull / delisting risk that violates the "treat each account's ruin as independent" premise.
