...
### Live Architecture (A1 active; A2, A3, A4 retired)
**The book is A1.** A2 retired 2026-09-25 (OOS Calmar 0.90 under the gate at the anchored phase; bug-free −6.3% vs SPY +15.6% since March with live tracking the synthetic — the strategy, not the harness; `HISTORY.md` 2026-09-25). A3 (2026-04-21) and A4 (2026-07-13) are retired; slots are preserved for successor strategies (candidates: A5-Events pooled event sleeve, VIX tail leg, basis carry — see `docs/archive/AUDIT_FABLE.md` §5).

- **Account 1 (FIRE 0.1 — Momentum)**: SM + SPY Filter + book-level vol-scaling (cap 1.5, added 2026-07-18) — profits when trends persist. Every-21-trading-days rebalance, automated (cron). **Live-tracking clock: `LIVE_CLOCK_START = 2026-09-21`** (`data/trading_dates.py`) — the dashboard chart, SPY benchmark, performance table and scorecard cycles all start there; everything earlier was traded on stale rankings and is history, not evidence (`/api/portfolio/history?full=1` for forensics).
- **Account 2 (FIRE 0.2 — RETIRED 2026-09-25)**: Was 30% Multi-Asset Trend + 70% Low-Vol + vol-scaling. Liquidated 09-25 (fills at the 09-28 open; one non-tradeable CVR remains). Shadow-tracked weekly. DBMF swap parked under its 2022-class-event trigger.
...
- **Account 4 (FIRE 0.4 — RETIRED 2026-07-13)**: Was Crypto Momentum Rotation (top 2 of 9 coins, BTC 125d SMA filter + vol-scaling, daily cron). Retired on three legs: walk-forward edge decay (newest window +9.6% CAGR, Calmar 0.87 < 1.0 gate), +17.9pp live drag traced to bug-era churn and never re-validated live, unreachable 40%-upgrade clock. The 33%→40% pre-commitment is void. Canonical narrative: `HISTORY.md` 2026-07-13. Cron entry removed 2026-07-18; scripts remain for a successor. Shadow-tracked weekly in `live_scorecard.py` (reopen threshold: shadow > +10%).

A1 OOS (revalidated 2026-09-25: anchored re-ranking phase, SPY filter lagged one day, financing modeled): PASS 32.1%/Calmar 2.02 (SPY over the same 2023→ window: 22.0%/1.19). The book is 100% A1; no book split to maintain. Realistic live estimate: A1 Calmar ~2 (correlations spike in crises; calm-regime extension to 1.5× scales losing days too). Historical combined numbers are in git history and `HISTORY.md`.

Multi-account credentials in `.env` (`ALPACA_API_KEY` + `_2`/`_3`/`_4`). `AlpacaBroker(account=1|2|3|4)` selects. Rebalance schedule: A1 every 21 trading days anchored to 2026-04-21 — **automated since 2026-09-09** (`scripts/scheduled_rebalance.py`, cron every 10 min `--if-due`, trades in the last 75 min before the close on rebalance days — **the laptop must be awake for at least one 10-minute slot in that hour** (09-21: first automated day lost to a closed lid at the then-hourly fire); `data/trading_dates.py` is the calendar), filter monitor every 4h (cron), live scorecard weekly (cron, Mon 09:00). See `AUTOMATION.md` for full operational detail. Upcoming A1 rebalance dates: 2026-10-20 (Tue), 2026-11-18 (Wed).

### Strategies

**See `STRATEGIES.md`** for full OOS scorecards, research/building-block tables, universe definitions, filters & overlays, and strategy source files. Quick status: A1 PASS (CAGR 32.1%, Calmar 2.02, revalidated 09-25 with the filter look-ahead removed; vol_target 0.18, financing modeled); A2 RETIRED 09-25, A3 RETIRED, A4 RETIRED. Open caveat: C3 (S&P 500 survivorship bias, ~1-2pp on A1).

### Risk Controls (summary — see `AUTOMATION.md` for operational detail)

- **Rebalance grid (2026-09-09):** every strategy re-ranks every 21 trading days phased to `REBALANCE_ANCHOR = "2026-04-21"` (`strategies/base.py`), on the bar *before* each live rebalance date — so the backtest's phase is live's phase and a rebalance on its scheduled day trades a one-day-old ranking. **Never slip a rebalance**: the ranking doesn't refresh until the next grid date, and the preview's "Ranking as of" line goes amber past 5 days. Before this fix live traded rankings 2-3 weeks stale every cycle (HISTORY.md 2026-09-09).
- **Backtest filters act the day after the close that flips them** (`run_portfolio`, shift(1) since 2026-09-25); the same-day version was a look-ahead worth +4.7pp on A1. **Equity caches hold settled rows only** (today's row is dropped on write before 16:05 ET). **Equity snapshots are Alpaca's daily closes** (`backfill_from_alpaca` overwrites; cron `take_snapshot` only after the close) — intraday snapshots had put ±0.5% of noise into every live-vs-signal number. All three: HISTORY.md 2026-09-25.
...
- **Shared price caches download from canonical floors** (`download_and_cache` 2005, `download_sp500_prices` `SP500_CACHE_FLOOR` 2010, crypto 2020/2018) regardless of the caller's `start`, and a cached frame that starts after its floor is refreshed, not served. `start` never slices a cache hit — slice the result yourself. Two incidents (08-10 etf_prices, 09-08 sp500_prices) came from short-lookback callers rewriting a shared cache.
...
**Status (2026-08-12 — public, and honest about the margin):** The repo is **public** as of today — github.com/gdb-mtx/jim, public name **Jim** (FIRE never delivered its namesake; Jim Simons' system worked — LOL, but it's in the README). Take-public receipts: `docs/archive/PUBLIC_PLAN.md`; pre-scrub history survives privately in `gdb-mtx/fire-archive` (never publish). The August work made July's sizing honest: signal-book vol scalar (bit-for-bit backtest parity, replacing the drifting account-equity estimator), margin financing modeled at 5.5%/yr in every backtest, vol_target 0.15→0.18 (juice grid; cap raise past 1.5 rejected). Revalidated: **A1 PASS 33.0%**, **A2 MARGINAL 12.9%** — A2's old 16.8% PASS booked ~3pp of free margin; Q4 kill/swap trigger stands. Platform hardened 08-08..12 after three cache incidents (hot-path audit — `DATA_SOURCES.md`). Two alert-only signals watch the book (VIX tail, macro composite — runbook in `AUTOMATION.md`); travel watcher disabled (no daily-cadence client). George's standing posture unchanged: bold builds and honest kills over defensive maintenance; the Q3 real-money checkpoint rides on clean rebalance cycles (cycle 1 closed 08-20 for A1 only — A2 was silently blocked 08-14→09-09 by the EA-merger reconciliation bug; the planned 70/30 paper reset was never done and is now deliberately off the table, see "Book split" above).
...
- Account 1: 15 stocks (SM + SPY Filter + book vol-scaling since 07-18, vol_target 0.18 since 08-12) — live since 2026-03-10, OOS CAGR 32.1%. **Clean-window drag was the stale-ranking bug** (HISTORY.md 2026-09-09): at the right grid phase the signal beat SPY +1.7pp over 04-21→09-09 while live lost 6.2pp; from 09-21 live trades the same book the backtest does.
- Account 2: RETIRED 2026-09-25 (see `HISTORY.md`). The Q4 kill trigger executed early on structural evidence.
...
**Next steps:**
- **A1 program (2026-09-25, the book is A1):** (1) two clean automated cycles first — 09-21→10-20→11-18 — as the parity proof before any live change; (2) sign-aware vol scaling — no leverage extension below the 200d or inside a drawdown deeper than X (A2 died levered 1.5× on a losing low-vol book; A1 has the same exposure), backtest it; (3) C3 survivorship becomes load-bearing with A1 as the whole book; skip parameter tuning — Test 6 says defaults are within 10% of the refit. Scorecard reads from the 09-21 clock; first cycle closes 10-20. The Monday scorecard cron is the Q3-checkpoint evidence trail (gate: A1 clean-window alpha ≥ 0 vs SPY over 2-3 cycles). **Scorecard 09-09 reads:** A1 cumulative alpha −6.2% since 04-21 with live trailing its own signal by −8.5pp (−2.9pp in the 07-22→08-20 cycle alone) — the drag class is back and needs a signal_tracker-style decomposition before the Q3 call; **shadow A4 +16.6% since retirement (BTC +25.8%), past the +10% reopen threshold** — the slot conversation is due.
- **A4 fresh look 09-10:** stays retired — walk-forward decay continued into a sixth window (+3.8%, Calmar 0.26); the shadow trigger now needs ≥126 days (HISTORY.md 2026-09-10).
- **Standing George decisions:** VIXY tail leg REAL-money adoption (paper pilot auto-executes in A3 since 07-18; judge on its live record at Q4). Parked-with-triggers ledger lives in the `docs/archive/AUDIT_FABLE.md` banner: basis carry (funding >8-10%), DBMF A2 swap (A2 live Calmar <1.0 at Q4), SPY put-ladder tail-hedge comparison (after pilot's first realized round trip), IBKR bundle ($150K+ real money), micro-cap QM (point-in-time data). Basis carry stays parked until Deribit 30d funding > ~8-10% annualized (monthly one-command probe in `EVENT_KILLTESTS_JUL2026.md`; also worth checking when BTC re-crosses its 125d SMA).
- **Fly.io deployment** (`DEPLOYMENT_PLAN.md`): the durable fix for laptop scheduling and the real-money gate. Parked by George pending the real-money commitment.
- **Hygiene:** tests/ repaired 07-18 (131 green; conftest isolates real state). Win-rate 0.0% report bug fixed 07-18. C3 survivorship: Norgate declined 2026-07-18 (George: not worth $30/mo while edges are the constraint); handle pre-real-money by haircutting A1 expectations ~1-2pp or a free sensitivity test.
