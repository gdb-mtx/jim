...
- **Standing George decisions:** VIXY tail leg REAL-money adoption (paper pilot auto-executes in A3 since 07-18; judge on its live record at Q4). Parked-with-triggers ledger lives in the `AUDIT_FABLE.md` banner: basis carry (funding >8-10%), DBMF A2 swap (A2 live Calmar <1.0 at Q4), SPY put-ladder tail-hedge comparison (after pilot's first realized round trip), IBKR bundle ($150K+ real money), micro-cap QM (point-in-time data). Basis carry stays parked until Deribit 30d funding > ~8-10% annualized (monthly one-command probe in `EVENT_KILLTESTS_JUL2026.md`; also worth checking when BTC re-crosses its 125d SMA).
