...
Multi-account credentials in `.env` (`ALPACA_API_KEY` + `_2`/`_3`/`_4`). `AlpacaBroker(account=1|2|3|4)` selects. Rebalance schedule: A1/A2 every 21 trading days anchored to 2026-04-21 — **automated since 2026-09-09** (`scripts/scheduled_rebalance.py`, hourly cron `--if-due`, fires in the last 75 min before the close on rebalance days; `data/trading_dates.py` is the calendar), filter monitor every 4h (cron), live scorecard weekly (cron, Mon 09:00). See `AUTOMATION.md` for full operational detail. Upcoming A1/A2 rebalance dates: 2026-09-21 (Mon), 2026-10-20 (Tue). (A2 missed 08-20 — blocked by the EA-merger reconciliation bug, `HISTORY.md` 2026-09-09 — and was rebalanced off-cycle on 09-09.)
