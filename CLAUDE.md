...
Factor diversification across Alpaca paper accounts. A3 (2026-04-21) and A4 (2026-07-13) are retired; both slots are preserved for successor strategies (candidates: A5-Events pooled event sleeve, VIX tail leg, basis carry — see `docs/archive/AUDIT_FABLE.md` §5).
...
Per-account OOS (revalidated 2026-09-09 at the anchored re-ranking phase, financing modeled): A1 PASS 34.6%/Calmar 2.59, A2 MARGINAL 11.8%/Calmar 0.90. No fresh combined number for the current configs; the old 50/50 figure (24.1%, pre-financing-model) is superseded. **Book split:** 70/30 A1/A2 is the real-money funding allocation. The paper accounts sit ~50/50 by equity (~$105K / ~$99K, 2026-09-09) and are deliberately never reset — the scorecard reads raw equity, so a reset would print as a fake ±38% cycle; `live_scorecard.py` scores the 70/30 book synthetically from per-account returns instead. Realistic live estimate: A1 Calmar ~2 (correlations spike in crises; calm-regime extension to 1.5× scales losing days too). Historical combined numbers are in git history and `HISTORY.md`.

Multi-account credentials in `.env` (`ALPACA_API_KEY` + `_2`/`_3`/`_4`). `AlpacaBroker(account=1|2|3|4)` selects. Rebalance schedule: A1/A2 every 21 trading days anchored to 2026-04-21 — **automated since 2026-09-09** (`scripts/scheduled_rebalance.py`, cron every 10 min `--if-due`, trades in the last 75 min before the close on rebalance days — **the laptop must be awake for at least one 10-minute slot in that hour** (09-21: first automated day lost to a closed lid at the then-hourly fire); `data/trading_dates.py` is the calendar), filter monitor every 4h (cron), live scorecard weekly (cron, Mon 09:00). See `AUTOMATION.md` for full operational detail. Upcoming A1/A2 rebalance dates: 2026-09-21 (Mon), 2026-10-20 (Tue). (A2 missed 08-20 — blocked by the EA-merger reconciliation bug, `HISTORY.md` 2026-09-09 — and was rebalanced off-cycle on 09-09.)

### Strategies

**See `STRATEGIES.md`** for full OOS scorecards, research/building-block tables, universe definitions, filters & overlays, and strategy source files. Quick status: A1 PASS (CAGR 34.6%, revalidated 09-09 at the anchored re-ranking phase, vol_target 0.18, financing modeled), A2 MARGINAL (CAGR 11.8%, Calmar 0.90 — under the gate on the anchored phase; the 07-18 16.8% PASS booked ~3pp of free margin; Q4 kill/swap trigger stands), A3 RETIRED, A4 RETIRED. Open caveat: C3 (S&P 500 survivorship bias, ~1-2pp on A1).
...
- Account 2: 34 positions (Trend + Low-Vol) — live since 2026-03-10. cap=1.5 restored 2026-07-18; MARGINAL 11.8% / Calmar 0.90 at the anchored phase (09-09; was 12.9% / 1.05 on 08-12). Live ~flat pre-restore; levered from the 07-22 rebalance; Q4 Calmar < 1.0 → kill or DBMF swap.
