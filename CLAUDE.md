...
Per-account OOS (revalidated 2026-08-12, financing modeled): A1 PASS 33.0%/Calmar 2.49, A2 MARGINAL 12.9%/Calmar 1.05. No fresh combined number for the current configs — the book resets to ~70/30 A1/A2 at the 08-20 rebalance; the old 50/50 figure (24.1%, pre-financing-model) is superseded. Realistic live estimate: A1 Calmar ~2 (correlations spike in crises; calm-regime extension to 1.5× scales losing days too). Historical combined numbers are in git history and `HISTORY.md`.

Multi-account credentials in `.env` (`ALPACA_API_KEY` + `_2`/`_3`/`_4`). `AlpacaBroker(account=1|2|3|4)` selects. Rebalance schedule: A1/A2 every 21 trading days anchored to 2026-04-21 (manual, ~3:00 PM ET), filter monitor every 4h (cron), live scorecard weekly (cron, Mon 09:00). See `AUTOMATION.md` for full operational detail. Upcoming A1/A2 rebalance dates: 2026-08-20 (Thu — George resets paper balances to 70/30, A1 ≈ $143K / A2 ≈ $61K, that morning; normal rebalance ~3 PM ET), 2026-09-21 (Mon).
...
**Next steps:**
- **Aug 20 rebalance (top priority):** George resets paper balances to 70/30 (A1 ≈ $143K / A2 ≈ $61K) in the Alpaca dashboard that MORNING, then normal rebalance ~3 PM ET. New sizing (signal-book scalar, vol_target 0.18) applies automatically. This closes Q3-checkpoint cycle 1 — the Monday scorecard cron is the evidence trail (gate: A1 clean-window alpha ≥ 0 vs SPY over 2-3 cycles).
